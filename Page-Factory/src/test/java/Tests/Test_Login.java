...
import PageFactory.DemoQA_Home;
import PageFactory.DemoQA_Login;
...
public class Test_Login {
    ...
    public void TestLogin(){
        ...
        try{
            ...
            wait.until(ExpectedConditions.visibilityOf(Login.getTitle()));
            Assert.assertEquals("Profile", Login.getTitle().getText());
    ...
    public void TestError(){
        Home.IrBookStore();
        Home.IrLogin();
        Login.IngresarUsuario("aarroyoc1999");
        Login.IngresaPassword("@Aaac_1999");
        Login.Login();
        try {
            WebDriverWait wait = new WebDriverWait(edge, Duration.ofSeconds(5));
            wait.until(ExpectedConditions.visibilityOf(Login.getMensajeError()));
            Assert.assertEquals("Invalid username or password!", Login.getMensajeError().getText());
        }catch (AssertionError error){
            System.out.println("Prueba Fail! = " + error);
        }
    }
